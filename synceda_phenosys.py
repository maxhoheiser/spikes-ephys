...
def plt_fit_normdist(data,ax=False,norm_fit=True):
    ...
    if ax == False:
        ...
        fig, ax = plt.subplots()
        # plot histogram
        ax.hist(data, bins=25, density=True, alpha=0.5)
    else:
        ax.hist(data, bins=25, density=True, alpha=0.5)
    # Plot the PDF.
    xmin, xmax = plt.xlim()
    ...
    if norm_fit:
        ...
        std=4
        p = norm.pdf(x, mu, std)
        ax.plot(x, p*0.8, "blue", linestyle='--', linewidth=3,label=f"center\nmu:{round_up(mu,2)}\nstd:{round_up(std,2)}")

        # right
        mu=22.04
        std=4
        p = norm.pdf(x, mu, std)
        ax.plot(x, p*0.1, "red", linestyle='--', linewidth=3,label=f"right\nmu:{round_up(mu,2)}\nstd:{round_up(std,2)}")
