"""
Generate all plots for Masterthesis
"""
...
thesis_folder = "/Users/max/Google Drive/3 Projekte/Masterarbeit Laborarbeit Neuroscience/Schreiben/Masterarbeit/masterthesis_latex"
...
def dump_session(data):
    folder=get_folder()
    with open(path.join(folder,'session.pkl'), 'wb') as file:
...
def save_fig(name, fig, chapter):
    folder = path.join(thesis_folder,chapter)+"/graphics"
    try:
        fig.savefig(
                    folder + "/" + name + ".pdf",
                    format='pdf',
...
def set_size(fraction=1,quad=False):
